                ...
                <div class="row">
                    <div class="col-lg-12">
                        <table class="table table-vcenter" id="retur_table">
                            ...
                            <tbody>
                                @foreach($transaksi->pembelian as $d)
                                    <tr>
                                        <td>
                                        ...
                                        <td>
                                            <div class="input-group input-number">
                                                ...
                                                <input type="number" class="form-control jumlah_beli" name="retur[{{ $d->id }}][jumlah]" value="{{ $d->qty_retur }}" min="1" max="{{ $d->quantity }}" data-max="{{ $d->quantity }}"  data-min="0">
                                        ...
                                        <td>
                                            <span class="row_subtotal display_currency" data-currency_symbol="true">{{ $d->qty_retur * $d->hrg_beli }}</span>
                                            <input type="hidden" class="form-control row_subtotal_hidden" name="retur[{{ $d->id }}][total]" value="{{ $d->qty_retur * $d->hrg_beli }}">
                                        </td>
                                    </tr>
                                @endforeach
                            </tbody>
                            <tfoot>
                                <tr class="table-warning">
                                    ...
                                    <td colspan="2" class="font-w700 text-right">
                                        <span class="display_currency total_subtotal" data-currency_symbol="true">{{ round($retur->jumlah_pengembalian) }}</span>
                                        <input type="hidden" id="total_subtotal_input" value="{{ round($retur->jumlah_pengembalian) }}"  name="sub_total">
                                    </td>
                                </tr>
                            </tfoot>
                        </table>
